...
export default function CompareCompaniesPanel({ companies, initialSelected = [], onClose }: CompareCompaniesPanelProps) {
    const [selected, setSelected] = useState<Company[]>(initialSelected);

    const options = useMemo<CompanyOption[]>(() => companies.map(c => ({ value: c.id, label: c.name, company: c })), [companies]);
    const value = useMemo<CompanyOption[]>(() => selected.map(c => ({ value: c.id, label: c.name, company: c })), [selected]);
    const series = useMemo(() => selected.map((c, i) => ({
        name: c.name, techStack: c.tech_stack, color: CHART_PALETTE[i % CHART_PALETTE.length],
    })), [selected]);

    const handleChange = (opts: readonly CompanyOption[]) => {
