package com.techpulse.techradar.features.agent.adapters.output;
...
public class PythonAgentClient implements AgentServicePort {
    ...
    @Value("${app.python.ai.timeout:120000}")
    ...
    @Override
    public Mono<Map<String, Object>> runAgent(Map<String, Object> request) {
        return webClient()
                .post()
                .uri("/agent")
                ...
                .onErrorResume(ex -> {
                    log.error("Agent service error", ex);
                    return Mono.error(new DatabaseUnavailableException("Agent service unavailable"));
                });
    }
}
