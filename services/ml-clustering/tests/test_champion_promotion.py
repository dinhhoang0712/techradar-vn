...
def test_promotes_when_no_champion_exists_yet(mock_mlflow):
    ...
    result = register_best_model(
        "run123", "tech-clusters", primary_metric="silhouette", primary_metric_value=0.5
    )
...
def test_promotes_when_new_silhouette_beats_champion(mock_mlflow):
    ...
    result = register_best_model(
        "run456", "tech-clusters", primary_metric="silhouette", primary_metric_value=0.6
    )
...
def test_keeps_old_champion_when_new_model_is_worse(mock_mlflow):
    ...
    result = register_best_model(
        "run789", "tech-clusters", primary_metric="silhouette", primary_metric_value=0.6
    )
...
def test_davies_bouldin_lower_is_better_new_wins_with_lower_value(mock_mlflow):
    ...
    result = register_best_model(
        "runabc", "tech-clusters", primary_metric="davies_bouldin", primary_metric_value=0.3
    )
...
def test_promotes_when_champion_run_missing_the_metric(mock_mlflow):
    ...
    result = register_best_model(
        "runxyz", "tech-clusters", primary_metric="silhouette", primary_metric_value=0.6
    )
