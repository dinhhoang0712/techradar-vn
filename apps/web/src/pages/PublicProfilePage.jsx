...
export default function PublicProfilePage() {
    ...
    const toggleFollow = async () => {
        ...
        setProfile((p) => ({ ...p, is_following: !wasFollowing, follower_count: Math.max(0, p.follower_count + (wasFollowing ? -1 : 1)) }));
        try {
            await (wasFollowing ? unfollowUser(id) : followUser(id));
        } catch (err) {
            setProfile((p) => ({ ...p, is_following: wasFollowing, follower_count: Math.max(0, p.follower_count + (wasFollowing ? 1 : -1)) }));
            notify({ title: 'Không thể cập nhật theo dõi', body: err.message, variant: 'error' });
        } finally {
            setFollowBusy(false);
        }
    };
    ...
    const handleMessage = async () => {
        ...
        try {
            ...
            notify({ title: 'Không thể mở cuộc trò chuyện', body: err.message, variant: 'error' });
        } finally {
