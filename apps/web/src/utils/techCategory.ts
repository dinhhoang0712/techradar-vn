...
export const CATEGORY_ORDER: string[] = [...CATEGORY_KEYWORDS.map(([name]) => name), OTHER_CATEGORY];

export function categorizeTech(techName?: string | null): string {
...
export function buildTechRadarData(techStack: string[] = []): TechRadarPoint[] {
    const counts = Object.fromEntries(CATEGORY_ORDER.map(c => [c, 0])) as Record<string, number>;
