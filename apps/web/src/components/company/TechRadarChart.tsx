...
export interface TechRadarSeries {
    name: string;
    techStack: string[];
    color: string;
}

function buildOverlayData(series: TechRadarSeries[]) {
...
function RadarTooltip({ active, payload, label }: Partial<TooltipContentProps<ValueType, NameType>>) {
...
export default function TechRadarChart({ series, height = 280 }: TechRadarChartProps) {
    if (!series?.length) return null;
    const data = buildOverlayData(series);
    const maxValue = Math.max(1, ...data.flatMap(row => series.map(s => Number(row[s.name]) || 0)));
