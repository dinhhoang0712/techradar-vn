...
def parse_args():
    parser = argparse.ArgumentParser(description="Cào dữ liệu Dân Trí để gán nhãn")
    ...
    parser.add_argument("--output-file", default="raw_data_DT_label.json", help="Tên file output")
...
for page in range(1, args.num_pages + 1):
    ...
    page_url = source_url if page == 1 else source_url.replace(".htm", f"/trang-{page}.htm")
    driver.get(page_url)
    time.sleep(2)
