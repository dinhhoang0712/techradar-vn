...
class GroqProvider:
    """SDK của Groq theo cùng khuôn với OpenAI (chat.completions), nên implementation gần như y hệt."""
    ...
    async def chat_stream(
        ...
        kwargs = self._apply_config(
            {"model": self.model, "messages": self._to_payload(messages), "stream": True},
            config,
        )
        try:
            ...
            async for chunk in stream:
                ...
                if getattr(chunk, "usage", None) or getattr(chunk, "x_groq", None):
                    usage_obj = chunk.usage or getattr(chunk.x_groq, "usage", None)
                    if usage_obj:
                        yield TokenUsage(
