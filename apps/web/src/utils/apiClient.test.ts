...
import type { apiClient as ApiClientFn } from './apiClient';
...
let apiClient: typeof ApiClientFn;
...
describe('apiClient — refresh-on-401', () => {
  it('refreshes the token once, stores it, and retries the original request', async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- mock needs an ad-hoc counter property
    const fetchMock: any = vi.fn((url: string) => {
      ...
      const status = fetchMock.protectedCalls++ === 0 ? 401 : 200;
      return Promise.resolve(jsonResponse(status, status === 200 ? { data: 'ok' } : {}));
    });
