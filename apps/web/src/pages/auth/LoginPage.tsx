...
type AuthModal = 'forgot' | 'reset' | null;
...
export default function LoginPage() {
    ...
    const handleForgotSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            notify({ title: 'Không gửi được yêu cầu', body: (err as Error).message, variant: 'error' });
        } finally {
            setModalBusy(false);
        }
    };
    ...
    const handleResetSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            notify({ title: 'Đặt lại mật khẩu thất bại', body: (err as Error).message || 'Mã sai hoặc đã hết hạn.', variant: 'error' });
        } finally {
            setModalBusy(false);
        }
    };
        ...
        try {
            ...
            if (res.access_token) {
                ...
                try {
                    ...
                    const withData = user as Partial<ApiResponse<User>> & Partial<User>;
                    const role = withData.data?.role ?? withData.role;
                    if (role) {
                        userRole = role;
                    }
                } catch (userError) {
