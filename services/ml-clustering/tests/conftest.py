...
# Tự động thêm đường dẫn module clustering (thư mục service) vào PYTHONPATH
ML_CLUSTERING_ROOT = Path(__file__).resolve().parents[1]
...
def dummy_data():
    ...
    gds_features = {
        ...
        "fastrp": pd.DataFrame(
            {"tech_id": ["t1", "t2", "t3", "t4"], "fastrp_0": [1, 2, 3, 4], "fastrp_1": [4, 3, 2, 1]}
        ),
    }
    ...
    graph_stats = pd.DataFrame({"tech_id": ["t1", "t2", "t3", "t4"], "degree": [10, 20, 30, 40]})
