...
import type { CmsContent } from '../../api/adminService';
...
type CmsFormTarget = Partial<CmsContent> | null;

export default function AdminCMS() {
    const [data, setData] = useState<CmsContent[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [formTarget, setFormTarget] = useState<CmsFormTarget>(null); // null = closed, {} = new, item = edit
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setError((e as Error).message || 'Không tải được dữ liệu CMS');
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const openEdit = (item: CmsContent) => setFormTarget({ ...item });
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            notify({ title: formTarget.id ? 'Cập nhật thất bại' : 'Tạo bản ghi thất bại', body: (e as Error).message, variant: 'error' });
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            notify({ title: 'Xoá thất bại', body: (e as Error).message, variant: 'error' });
        }
    };
