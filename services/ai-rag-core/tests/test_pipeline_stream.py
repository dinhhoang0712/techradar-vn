...
async def test_answer_stream_success(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.core.pipeline_stream.graph_search", AsyncMock(return_value={"jobs": [], "entities": ["Python"]})
    )
    monkeypatch.setattr("app.core.pipeline_stream.rerank", lambda query, candidates, top_k: candidates)
    monkeypatch.setattr(
        "app.core.pipeline_stream.build_messages", lambda *args, **kwargs: [{"role": "user", "content": "..."}]
    )
