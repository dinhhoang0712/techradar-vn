...
class EmbeddingService:
    ...
    def process_article(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "embedding": embedding,
            "metadata": {"title": data.get("title", ""), "published_date": data.get("publish_date", "")},
        }
    ...
    def send_vector(self, topic: str, vector_data: dict[str, Any]):
        ...
        try:
            future = self.producer.send(topic, key=vector_data.get("id"), value=vector_data)
...
def main():
    ...
    service = EmbeddingService(model_name=model_name, bootstrap_servers=bootstrap_servers)
    service.run()
