...
def mock_store():
    ...
    store.lookup_tech = lambda n: (
        store.name_lower_to_id.get(n.lower()),
        store.tech_to_cluster.get(store.name_lower_to_id.get(n.lower())),
    )
    ...
    def _save_cluster_override(cid, *, label=None, label_en=None, description=None, domain=None, actor=None):
        ...
        updated = {
            **store.cluster_labels[cid],
            **fields,
            "overridden": True,
            "overridden_by": actor,
            "overridden_at": "2026-07-18T00:00:00+00:00",
        }
...
def test_api_batch_predict_counts_provisional_separately_from_not_found(client, mock_store):
    ...
    batch = client.post("/predict/batch", json={"tech_names": ["Python", "Pythonn", "TotallyUnknown"]}).json()
