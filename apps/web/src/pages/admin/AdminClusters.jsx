...
function needsReview(cluster) {
    return !cluster.is_coherent || cluster.confidence < LOW_CONFIDENCE_THRESHOLD;
}
...
export default function AdminClusters() {
    ...
    const [form, setForm] = useState({ label: '', labelEn: '', description: '', domain: '' });
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setError(e.message || 'Không tải được danh sách cụm công nghệ');
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const reviewQueue = useMemo(
        () => filtered.filter(needsReview).sort((a, b) => a.confidence - b.confidence),
        [filtered]
    );
    const rest = useMemo(
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            notify({ title: 'Lưu thất bại', body: e.message, variant: 'error' });
        } finally {
            setSaving(false);
        }
    };
                    ...
                    {!detailLoading && detail && (
                        <div className="cluster-detail">
                            <div className="cluster-detail-meta">
                                <RingGauge percent={detail.confidence * 100} size={48} strokeWidth={5} label={`${Math.round(detail.confidence * 100)}%`} />
                            ...
                            <div className="cluster-members">
                                <strong>Thành viên ({detail.members.length}):</strong>
                                <div className="pill-group">
...
function ClusterCard({ cluster, flagged, onOpen }) {
    return (
        <button className={`cluster-card card${flagged ? ' flagged' : ''}`} onClick={onOpen}>
            <div className="cluster-card-top">
                ...
                <RingGauge percent={cluster.confidence * 100} size={36} strokeWidth={4} label={Math.round(cluster.confidence * 100)} />
            </div>
