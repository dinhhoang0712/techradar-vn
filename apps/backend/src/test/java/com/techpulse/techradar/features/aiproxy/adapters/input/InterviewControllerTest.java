...
class InterviewControllerTest {
    ...
    void setUp() {
        ...
        controller = new InterviewController(new AiProxyRequestHandler(aiProxyPort, activityLog, rateLimiter));
    }
    ...
    @Test
    void interview_forwardsToInterviewPath_withUserIdAttached_andWrapsResponse() {
        Map<String, Object> body = Map.of("history", List.of());
        Map<String, Object> proxyResponse = Map.of("question", "Tell me about yourself.");
        when(aiProxyPort.forward(eq("/interview"), any(), any(Duration.class))).thenReturn(Mono.just(proxyResponse));

        StepVerifier.create(controller.interview(body).contextWrite(authenticatedAs("user-1")))
                .assertNext(response -> {
        ...
        verify(aiProxyPort).forward(eq("/interview"), requestCaptor.capture(), any(Duration.class));
        assertThat(requestCaptor.getValue()).containsEntry("user_id", "user-1");
    }

    @Test
    void interview_returns503_whenInterviewServiceUnavailable() {
        when(aiProxyPort.forward(eq("/interview"), any(), any(Duration.class)))
                .thenReturn(Mono.error(new RuntimeException("connection refused")));

        StepVerifier.create(controller.interview(Map.of()).contextWrite(authenticatedAs("user-1")))
                .assertNext(response -> {
